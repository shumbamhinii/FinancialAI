...
const WhatIfScenariosTab: React.FC<WhatIfScenariosTabProps> = ({
  ...
  // Effect to synchronize editableProducts with the 'products' prop
  useEffect(() => {
    // This check ensures we only update if the 'products' prop actually changes significantly,
    // preventing unnecessary re-renders and simulation resets.
    if (JSON.stringify(editableProducts.map(p => ({name: p.name, price:p.price, expectedUnits: p.expectedUnits, costPerUnit: p.costPerUnit}))) !==
        JSON.stringify(products.map(p => ({name: p.name, price:p.price, expectedUnits: p.expectedUnits, costPerUnit: p.costPerUnit})))) {
      setEditableProducts(products.map(p => ({ ...p, costPerUnit: p.costPerUnit || 0 })));
      // Reset simulation data and results when products change to avoid showing old data for new products
      setSimulationChartData([]);
      setAllSimulatedPaths({});
      setSimulatedTotalRevenue(null);
      setSimulatedTotalProfit(null);
      setSimulatedMargin(null);
    }
  }, [products]); // Dependency on the products prop from the parent
